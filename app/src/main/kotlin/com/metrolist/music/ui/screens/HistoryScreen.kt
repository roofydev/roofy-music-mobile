...
fun HistoryScreen(
    ...
    fun dateAgoToString(dateAgo: DateAgo): String =
        when (dateAgo) {
            DateAgo.Today -> "--- ${stringResource(R.string.today).uppercase()} ---"
            DateAgo.Yesterday -> "--- ${stringResource(R.string.yesterday).uppercase()} ---"
            DateAgo.ThisWeek -> "--- ${stringResource(R.string.this_week).uppercase()} ---"
            DateAgo.LastWeek -> "--- ${stringResource(R.string.last_week).uppercase()} ---"
            is DateAgo.Other -> "--- ${dateAgo.date.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))} ---"
        }
                ...
                Row(
                    ...
                    filters.forEach { (source, label) ->
                        RetroButton(
                            ...
                            Text(
                                if (historySource == source) "[ $label ]" else "[ ] $label",
                                style = MaterialTheme.typography.labelSmall,
                                color = if (historySource == source) RetroTokens.TextHot else RetroTokens.TextMuted,
                            )
                        }
                    }
                }
