    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { },
                navigationIcon = {
                    RetroIconButton(onClick = onClose) {
                        Icon(painterResource(R.drawable.arrow_back), stringResource(R.string.back_button_desc), tint = RetroTokens.Text)
                    }
                },
                actions = {
                    RetroIconButton(onClick = { audioService.toggleMute() }) {
                        val icon = if (isMuted) R.drawable.volume_off else R.drawable.volume_up
                        Icon(painterResource(icon), "Mute", tint = RetroTokens.Text)
                    }
                },
